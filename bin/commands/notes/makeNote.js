...
async function makeNote(name) {
  ...
  if (await verifyDirectory(`${path}/Notes/${name}`, true)) {
    console.log(
  ...
  await makeDirectory(`${path}/Notes/${name}`);
  ...
  await copyDir(
    templateToCopy,
    getDirectoryPath(`${path}/Notes/${name}`),
    {},
    function(err) {
      if (err) {
        console.error(err);
        ...
        shell.cd(getDirectoryPath(`${path}/Notes/${name}`));

...
async function viewItem() {
  ...
  let stuff = await shell.exec(`find ${getDirectoryPath("").slice(0,-1)} -type d  -not -path '*/\.*';`, {silent: true}).stdout.split("\n").slice(0,-1)
  let formattedStuff = stuff.map(path => {
    let tempPath = path.replace(homedir + "/Documents", "").substring(1).split("/")
